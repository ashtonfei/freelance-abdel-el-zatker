...
const PAGE_NOT_FOUND = '<h1 class="text-center mt-4">Current Listings</h1>'
...
class App {
    ...
    getHtmlContentByUrl(url, details = false) {
        const response = UrlFetchApp.fetch(url)
        const code = response.getResponseCode()

        if (code !== 200) return null
        const content = response.getContentText()
        if (content.indexOf(PAGE_NOT_FOUND) !== -1 && details) return null
        return content
    }
    ...
    getDetails(url, imageUrl) {
        let content = this.getHtmlContentByUrl(url, true)
